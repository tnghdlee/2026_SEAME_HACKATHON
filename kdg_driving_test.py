...
class DrivingPolicy:
    ...
    def __init__(self, params=None):
        ...
        self.turn_intent = None       # 'left' | 'right' | None (갈림길별 확정 래치)
        self.fork_remaining = 0
        # 커밋 종료 직후 같은 표지판이 계속 보여 곧바로 재래치되는 것을 막는 잠금.
        # 표지판이 한 프레임이라도 사라진 뒤에야 다음 갈림길을 새로 받는다.
    ...
    def on_detections(self, detections):
        ...
        # 좌/우 margin 게이팅 후 스트릭 → turn_intent 래치(갈림길별 확정).
        ...
        if side == 'left':
            ...
            self._left_streak = 0
            self._right_streak = 0
            # 표지판이 사라짐 → 이전 커밋 잠금 해제, 다음 갈림길 재무장 허용.
            self._fork_lock = False

        # 새 분기 의도는 (1) 현재 진행 중인 의도가 없고 (2) 직전 커밋의 잠금이
        # 풀렸을 때만 받는다. 이로써 한 미션에서 갈림길이 여러 번 나와도
        # 각각 새로 확정된다(예전엔 첫 확정이 영구 래치돼 두 번째 갈림길을
        # 무시했다).
        if self.turn_intent is None and not self._fork_lock:
    ...
    def step(self, lane):
        ...
        if committing:
            ...
            # 커밋 창 종료 → 의도 해제 후 잠금. 표지판이 사라져야(on_detections
            # 에서 _fork_lock=False) 다음 갈림길을 새로 받는다. 이로써 표지판이
            # 커밋 창보다 오래 보여도 곧바로 재래치되는 stutter 를 막는다.
